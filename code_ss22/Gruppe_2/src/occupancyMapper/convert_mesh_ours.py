...
#sys.path.append( '/home/valentinh/spotSem/Place-Categorization-2D/')
#try:
#    from place_categorization_ours import place_categorization_ours as plcat
#except:
...
def load_ply_file ( file_name, print_info=False ):
    ...
    Vr = 1#ply_data['vertex'].data['red'] # u1 -> usigned int 8
    Vg = 1#ply_data['vertex'].data['green'] # u1 -> usigned int 8
    Vb = 1#ply_data['vertex'].data['blue'] # u1 -> usigned int 8
    Va = 1#ply_data['vertex'].data['alpha'] # u1 -> usigned int 8

    faces = 1#np.array([ face[0] for face in ply_data['face'] ])
...
################################################################################
def slice_horizontal_vertices(Vz, config):
    '''
...
################################################################################
def convert_2d_pointcloud_to_ogm (Vx,Vy, ver_sliced_idx, config, posX, posY):
    '''
    ...
    #build a circle with spots with as diameter
    #TODO optimize this for only uneven spot_size_px
    spot_size = 0.6  ## the smallest side of spot (width) in m 
    spot_size_px = int(spot_size/mpp)
    spot_size_mask = np.zeros((spot_size_px, spot_size_px), dtype=np.uint8)
    rr, cc = disk((spot_size_px/2,spot_size_px/2),spot_size_px/2)
    spot_size_mask[rr, cc] = 1
    #print(spot_size_mask)

    #rroff = rr-int(spot_size_px/2)
    #ccoff = cc-int(spot_size_px/2)
    margin = int(spot_size_px/2)
    ...
    OffPxX = ((-Vx[idx].min())/mpp).astype(np.uint32) + margin
    OffPxY = ((-Vy[idx].min())/mpp).astype(np.uint32) + margin

    vis_row_idx = ((posY - Vy[idx].min()) /mpp).astype(np.uint32) + margin
    vis_col_idx = ((posX - Vx[idx].min()) /mpp).astype(np.uint32) + margin

    curPosX = vis_col_idx[-1]
    curPosY = vis_row_idx[-1]

    if fill_neighbors:
        #for r in rr:
        #    for c in cc:
        #        if spot_size_mask[r,c] == 1:
        #            row_idx = np.stack((row_idx, row_idx+r-int(spot_size_px/2)), axis=0)
        #            col_idx = np.stack((col_idx, col_idx+r-int(spot_size_px/2)), axis=0)
        ...
        col_idx = np.stack( (col_idx-1, col_idx, col_idx+1,
                             ...
                             col_idx-1, col_idx, col_idx+1)
                            , axis=0 )
    vis_row_idx = np.stack( (vis_row_idx-2, vis_row_idx-2, vis_row_idx-2, vis_row_idx-2, vis_row_idx-2,
                             vis_row_idx-1, vis_row_idx-1, vis_row_idx-1, vis_row_idx-1, vis_row_idx-1,
                             vis_row_idx,   vis_row_idx,   vis_row_idx,   vis_row_idx,   vis_row_idx,
                             vis_row_idx+1, vis_row_idx+1, vis_row_idx+1, vis_row_idx+1, vis_row_idx+1,  
                             vis_row_idx+2, vis_row_idx+2, vis_row_idx+2, vis_row_idx+2, vis_row_idx+2), axis=0)

    vis_col_idx = np.stack( (vis_col_idx-2, vis_col_idx-1, vis_col_idx,   vis_col_idx+1, vis_col_idx+2,
                             vis_col_idx-2, vis_col_idx-1, vis_col_idx,   vis_col_idx+1, vis_col_idx+2,
                             vis_col_idx-2, vis_col_idx-1, vis_col_idx,   vis_col_idx+1, vis_col_idx+2,
                             vis_col_idx-2, vis_col_idx-1, vis_col_idx,   vis_col_idx+1, vis_col_idx+2,  
                             vis_col_idx-2, vis_col_idx-1, vis_col_idx,   vis_col_idx+1, vis_col_idx+2), axis=0)
    ...
    ogm[vis_row_idx,vis_col_idx] = 0.5
    ...
    return (ogm * 255).astype(np.uint8), mpp, OffPxX, OffPxY, curPosX, curPosY
