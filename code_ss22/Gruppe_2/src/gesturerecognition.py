...
class mvc:
  """enum for possible detectec gestures aka movementcommands aka mvc
  """
  NONE = 0
  HALT = 1
  COME = 2
...
def gestrec(mpHandIn):
  """Processes an mediapipe hand and returns a possible gesture
  Args:
  ...
  """
  if(type(mpHandIn) != mp.framework.formats.landmark_pb2.LandmarkList and type(mpHandIn) != mp.framework.formats.landmark_pb2.NormalizedLandmarkList):
  ...
  p2 = np.array((mpHandIn.landmark[2].x, mpHandIn.landmark[2].y, mpHandIn.landmark[2].z))       # thumb the one 2 before the tip 
  p3 = np.array((mpHandIn.landmark[3].x, mpHandIn.landmark[3].y, mpHandIn.landmark[3].z))       # thumb the one before the tip 
  ...
  if(scalprd > 0.9):
    minfinlen = min(np.linalg.norm(p0[0:2] - p8[0:2]),np.linalg.norm(p0[0:2] - p12[0:2]),np.linalg.norm(p0[0:2] - p16[0:2]))
    if(minfinlen > 1.5*np.linalg.norm(p0[0:2] - p5[0:2])):#detect halt command
      print("HALT")
      return mvc.HALT

    ## up command: thumb up
    maxfinlen = max(np.linalg.norm(p0[0:2] - p8[0:2]),np.linalg.norm(p0[0:2] - p12[0:2]),np.linalg.norm(p0[0:2] - p16[0:2]))
    if(maxfinlen < 1.3*np.linalg.norm(p0[0:2] - p5[0:2]) and (p3[1]-p4[1]) > 0.5*(p2[1]-p3[1]) and (p3[1]-p4[1]) > 0.5*np.linalg.norm(p3[0:2]-p2[0:2])):  # note to future self: the missing abs is correct
  ...
  ## come command: index and thumb ring, prooven by small distance between the two tips in 2D image
  if(np.linalg.norm(p4[0:2] - p8[0:2]) < 0.15 * np.linalg.norm(p0[0:2] - p5[0:2]) and scalprd > 0.6):
    print("COME")
    return mvc.COME
  return mvc.NONE
# Umgang mit fehlenden Punkten an der Handbasis notwendig ??
