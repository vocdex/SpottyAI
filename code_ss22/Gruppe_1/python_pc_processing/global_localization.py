...
import extract_point_cloud
import matplotlib.pyplot as plt
import ply_processing

ROBOT_IP = "192.168.80.3"
GRAPH_PATH = "./"

#TODO: instead of relying on majority vote, take the difference in pixel sums for different orientations of the matching img
# -> case with largest difference gets the highest weight
...
class RecordingInterface(object):
    ...
    def run(self):
        ...
        PDF_PATH = 'OG3.pdf'#"floor_map.pdf"
        EXTRACT_PLY_PATH_MATLAB = "output.ply"
        PREPROCESSED_PLY_PATH = "preprocessed.ply"
        voting_list = []
        pixle_voting_list = []
        PIXL_TOLERANCE = 50
        ...
        print(f"START LOCALIZATION")
        idx = 0
        while True:
            idx += 1
            print()
            print(f"TRIAL RUN #{idx}:")

            print("anchoring optimization")
            try:
            ...
            #TODO: extract point cloud
            EXTRACT_PLY_PATH = f"output_{idx}.ply"

            print("extract pc")
            extract_point_cloud.main(['--path', f"./downloaded_graph/{GRAPH_PATH}", '--output', EXTRACT_PLY_PATH])
            print("extract pc")
            extract_point_cloud.main(['--path', f"./downloaded_graph/{GRAPH_PATH}", '--output', EXTRACT_PLY_PATH_MATLAB])



            #TODO: Run MATLAB script -> use #PREPROCESSED_PLY_PATH
            print("run MATLAB preprocessing")
            ply_processing.run_MATLAB_preprocess()

            print("run localize")
            try:

                origin_on_map, relative_min_sum = ply_processing.localize(PDF_PATH, PREPROCESSED_PLY_PATH)# EXTRACT_PLY_PATH)

                print(f"origin_on_map: {origin_on_map}")
                print(f"relative_min_sum: {relative_min_sum}")

                if len(voting_list) == 0:
                    voting_list.append(1)
                    pixle_voting_list.append([origin_on_map[0] - PIXL_TOLERANCE, origin_on_map[0] + PIXL_TOLERANCE,
                                        origin_on_map[1] - PIXL_TOLERANCE, origin_on_map[1] + PIXL_TOLERANCE])
                else:

                    j = 0
                    while(True):
                        yx_list = pixle_voting_list[j]
                        if ((yx_list[0] < origin_on_map[0]) and (origin_on_map[0] < yx_list[1]) and
                             (yx_list[2] < origin_on_map[1]) and (origin_on_map[1] < yx_list[3])):
                            voting_list[j] += 1
                            break
                        j += 1

                        if(j == len(voting_list)):
                            voting_list.append(1)
                            pixle_voting_list.append([origin_on_map[0] - PIXL_TOLERANCE, origin_on_map[0] + PIXL_TOLERANCE,
                                                      origin_on_map[1] - PIXL_TOLERANCE, origin_on_map[1] + PIXL_TOLERANCE])
                            break
            ...
            if idx == 3:
                print()
                print("RESULT:")
                print(f"voting_list: {voting_list}")
                print(f"pixel_voting_list: {pixle_voting_list}")

                get_origin = np.argmax(voting_list)
                best_origin = (pixle_voting_list[get_origin][0]+PIXL_TOLERANCE, pixle_voting_list[get_origin][2]+PIXL_TOLERANCE)
                final_map = ply_processing.draw_final_map(best_origin, path='OG3.pdf')
