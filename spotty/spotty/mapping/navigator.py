...
class NavigationResult:
    ...
    def __init__(self, status: NavigationStatus, destination: Optional[str] = None, error_message: Optional[str] = None):
        self.status = status
        ...
        try:
            ...
            if not current_waypoint:
                return NavigationResult(
                    NavigationStatus.ROBOT_NOT_READY,
                    error_message="Robot is not localized"
                )
            
            # Select destination using strategy
            destination = command.strategy.select_destination(
                current_waypoint, 
                command.matching_waypoints
            )
            
            if not destination:
                return NavigationResult(
                    NavigationStatus.NO_PATH,
                    error_message="No valid path to any matching waypoint"
                )
