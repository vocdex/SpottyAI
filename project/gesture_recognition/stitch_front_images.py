"""
File for stitching 'frontleft_fisheye_image' and 'frontright_fisheye_image' together.

Code is based on the boston dynamics example "Stitch Front Images":
https://dev.bostondynamics.com/python/examples/stitch_front_images/readme

Main changes:
1.  The provided example uses pygame to display both images and then performs the stitching.
    Problem: Pygame always has to be performed as a visible frame on display screen.
    Solution: The new inserted function openGL_without_display() executes openGL (which is used for Pygame and
    stitching), but there is no need for any graphic display (and therefore no need for Pygame anymore).
2.  Major changes in function stitch() to be able to use the stitched image in OpenCV and respectively in MediaPipe
3.  bug fix: added a for-loop in function draw_routine()
...
def draw_routine(display, image_1, image_2, program):
    ...
    for i in range(0, 2):  # because one image isn't flipped properly
...
def openGL_without_display():
    """opens openGL without showing display"""
    # Credits:
    # https://shintaroshiba.medium.com/saving-3d-rendering-images-without-displays-on-python-opengl-f534a4638a0d
...
def stitch(image_1, image_2, vert_shader, frag_shader):
        ...
        display_size = (1080, 720)  # must be same as in openGL_without_display() chosen
        ...
        draw_routine(display_size, image_1, image_2, program)

        image_buffer = glReadPixels(0, 0, display_size[0], display_size[1], GL_RGB, GL_UNSIGNED_BYTE)
        image = numpy.frombuffer(image_buffer, dtype=numpy.uint8).reshape(display_size[1], display_size[0], 3)
...
def stitch_front_images(images):
    ...
    if not front_right or not front_left:
        print("Test7: no front_right or front_left")
        return False

    with open(os.path.join(os.path.dirname(__file__),'shader_vert.glsl'), 'r') as file:
        vert_shader = file.read()
    with open(os.path.join(os.path.dirname(__file__),'shader_frag.glsl'), 'r') as file:
        frag_shader = file.read()

    image = stitch(front_right, front_left, vert_shader, frag_shader)

    return image


openGL_without_display()