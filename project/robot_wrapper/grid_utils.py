...
def get_terrain_markers(local_grid_proto):
    ...
    for local_grid in local_grid_proto:
        if (
            local_grid.local_grid_type_name == "terrain"
        ):  # TODO: Support parsing the terrain_valid and the intensity fields in the proto
            vision_tform_local_grid = get_a_tform_b(
                local_grid.local_grid.transforms_snapshot,
                VISION_FRAME_NAME,
                local_grid.local_grid.frame_name_local_grid_data,
            ).to_proto()
